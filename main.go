...
func eventHandler(evt interface{}) {
	...
	case *events.Message:
		if v.Info.MediaType == "image" && strings.ToLower(v.Message.ImageMessage.GetCaption()) == "stickerize" {
			...
			os.MkdirAll(newpath, os.ModePerm)
			path := fmt.Sprintf("images/raw/%s%s", v.Info.ID, exts[0])
			outputPath := fmt.Sprintf("images/converted/%s%s", v.Info.ID, ".webp")
			...
			client.SendMessage(v.Info.Chat, "", msg)
			client.SendMessage(v.Info.Chat, "", completed)
		}
	}
}
